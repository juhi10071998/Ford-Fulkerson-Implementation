...
/*!
 * \brief This Function Implements the DFS algorithm.
 * \param[in] graph is the vector of vectors of pair to serve as the adjacency list.
 * \param[in] number is the number of nodes.
 * \param[in] s is the source vertex.
 * \param[in] visited vector to keep a track of nodes encountered in the path from the source
 * \details This function marks all the nodes which are reachable from the source vertex  \n
 */
void dfs(vector<vector <pair<long long,long long> > > graph, long long number, int s,vector<long long>& visited){
	visited[s]=1;
	for(long long i=0;i<graph[s].size();i++){
		if(visited[graph[s][i].first]==0)
		dfs(graph,number,graph[s][i].first,visited);
	}
}
/*!
 ...
 */
long long ford_fulkerson(vector<vector <pair<long long,long long> > > graph,long long s, long long t, long long number){
...
while(bfs(rgraph,number,s,t,parent)){
	...
	vector<long long> visited(number,0);

	/* to find the min cut, we call teh dfs to mark reachable nodes, then the edge pairs in which the source is marked visited and the destination not visited are printed*/
	dfs(rgraph,number,s,visited);
	for(i=0;i<number;i++){
		for(j=0;j<graph[i].size();j++){
			if(visited[i] && !visited[graph[i][j].first]){
				cout << i << "->" << graph[i][j].first << " w=" << graph[i][j].second << endl;
			}
		}
	}
...
int main(){
	...
	cout << "result is: " << endl;
	long long res = ford_fulkerson(graph,src,dest,n);
	
	cout << "min s-t cut is " << res << endl;
	return 0;
}
/*
